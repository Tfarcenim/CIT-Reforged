...
import schm.shsupercm.citresewn.defaults.config.CITResewnDefaultsConfig;
...
import schm.shsupercm.citresewn.util.logic.Loops;
   ...
   // @Entrypoint(CITTypeContainer.ENTRYPOINT)
        ...
        public void load(List<CIT<TypeEnchantment>> parsedCITs) {
            ...
            for (CIT<TypeEnchantment> cit : loaded)
                for (GlintRenderLayer glintLayer : GlintRenderLayer.values()) {
                    ...
                    Minecraft.getInstance().renderBuffers().bufferSource().fixedBuffers.put(renderLayer, /*? <1.21 {*//*new BufferBuilder(renderLayer.getExpectedBufferSize()) *//*?} else {*/ new ByteBufferBuilder(renderLayer.bufferSize()) /*?}*/);
                }
        }
    ...
    public enum GlintRenderLayer {
        ...
        /*ARMOR_GLINT("armor_glint", 8f, layer -> layer
                ...
                .cull(RenderStateShard.NO_CULL)
                .depthTest(RenderStateShard.EQUAL_DEPTH_TEST)
        ...
        *//*?}*/
        ...
        public RenderType build(TypeEnchantment enchantment, ResourceLocation propertiesIdentifier) {
            ...
            return RenderType.create("citresewn:enchantment_" + this.name + ":" + propertiesIdentifier.toString(),
