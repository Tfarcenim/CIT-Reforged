    ...
    public static ResourceLocation resolveAsset(ResourceLocation rootIdentifier, String path, String defaultedTypeDirectory, String extension, ResourceManager resourceManager) {
        if (path == null) {
            ...
            if (!path.endsWith(extension))
                path = path + extension;
            ResourceLocation pathIdentifier = ResourceLocation.fromNamespaceAndPath(rootIdentifier.getNamespace(), path);
            return resourceManager.getResource(pathIdentifier).isPresent() ? pathIdentifier: null;
        }

        ResourceLocation pathIdentifier = ResourceLocation.tryParse(path);
        ...
        if (path.startsWith("./"))
            ...
            pathIdentifier = ResourceLocation.fromNamespaceAndPath(pathIdentifier.getNamespace(), path);
            if (resourceManager.getResource(pathIdentifier).isPresent())
                return pathIdentifier;
            else if (path.startsWith("assets/")) {
                path = path.substring(7);
                int sep = path.indexOf('/');
                pathIdentifier = ResourceLocation.fromNamespaceAndPath(path.substring(0, sep), path.substring(sep + 1));
                if (resourceManager.getResource(pathIdentifier).isPresent())
                    return pathIdentifier;
            }
            pathIdentifier = ResourceLocation.fromNamespaceAndPath(pathIdentifier.getNamespace(), defaultedTypeDirectory + "/" + path);
            if (resourceManager.getResource(pathIdentifier).isPresent())
                return pathIdentifier;
        }
        ...
        pathIdentifier = ResourceLocation.fromNamespaceAndPath(rootIdentifier.getNamespace(), path);
