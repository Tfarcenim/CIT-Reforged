...
public final class PackParser { private PackParser() {}
        ...
        for (PackResources pack : resourceManager.listPacks().toList())
            for (String namespace : pack.getNamespaces(PackType.CLIENT_RESOURCES))
                for (String root : ROOTS) {
                    ResourceLocation identifier = ResourceLocation.fromNamespaceAndPath(namespace, root + "/cit.properties");
                    try {
                        ...
                        CITResewnFabric.logErrorLoading("Errored while loading global properties: " + identifier + " from " + pack./*? <1.21 {*//*getName*//*?} else {*/packId/*?}*/());
                        e.printStackTrace();
                    }
                }
        return globalProperties;
    ...
    public static List<CIT<?>> parseCITs(ResourceManager resourceManager) {
        ...
        for (String root : ROOTS) {
            for (Map.Entry<ResourceLocation, Resource> entry : resourceManager.listResources(root + "/cit", s -> s.getPath().endsWith(".properties")).entrySet()) {
                ...
                try {
                    ...
                    CITResewnFabric.logErrorLoading(e.getMessage());
                } catch (Exception e) {
                    CITResewnFabric.logErrorLoading("Errored while loading cit: " + entry.getKey() + (packName == null ? "" : " from " + packName));
                    e.printStackTrace();
                }
            }
        }
